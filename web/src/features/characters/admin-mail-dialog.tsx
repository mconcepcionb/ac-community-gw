...
import { azerothAdminCharactersMailMutation } from "@/api";
...
/** AdminMailDialog sends mail to any character as staff. */
export function AdminMailDialog({ character, trigger }: { character: string; trigger: ReactNode }) {
  const [open, setOpen] = useState(false);
  const form = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: emptyValues });
  const mutation = useMutation(azerothAdminCharactersMailMutation());
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    try {
      await mutation.mutateAsync({
        path: { name: character },
        body: { character, subject: values.subject, body: values.body, money, items },
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Delivers items and/or money to the character.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
